    ...
    var $ = jQuery,
        ...
        uploader = WebUploader.create({
            ...
            // 文件接收服务端。
            server: "./upload.php?table="+table,//你的后台图片上传接受地址 也就是uploadurl,  //  这里是图片上传地址
            // 选择文件的按钮。可选。
    ...
    uploader.on('uploadSuccess', function(file, response) {
       ...
       // console.log('图片地址:' + $fileUrl);
        ...
        $img.attr('table', "order");
        $img.attr("tableId",12);
        removefiles(file); // 删除文件
        removehttpfiles(response);
        // 传值赋值
        // 商品详细图片 位置
       // 这里自己赋值 传值
    });
    ...
    // 删除服务器文件
    function removehttpfiles(response) {
    	//console.log(response);
        // 删除本条数据
        // $('.delimgbtns').each(function(index, el) {
        $('.delimgbtns').click(function() {
